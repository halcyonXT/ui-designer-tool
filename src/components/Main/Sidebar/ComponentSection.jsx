    ...
    return (
        <>
            <div
                ...
                id={props.item._id}
                style={{
                    ...CREATE_STYLES.selected(
                        props.selected && subcomponents.selected.value === null
                    ),
                }}
            >
                ...
                <div className="-sidebar-component-icons-wrapper">
                    ...
                    {(props.selected || hovered) && (
                        <IconAction x={addSubcomponent}>{ICONS.add}</IconAction>
                    )}
                    {
                        <IconAction
                            x={collapseTrigger}
                    ...
                    <div className="-ICON" style={{ height: "80%" }}>
                        <IconAction
                            x={() => components.moveItem("up", props.item._id)}
                            xs={{ height: "160%", transform: "rotate(-90deg)", opacity: props.isFirst ? "0.5" : "1", pointerEvents: props.isFirst ? "none" : "all"}}
                        >
                            {ICONS.expand}
                        </IconAction>
                        <IconAction
                            x={() => components.moveItem("down", props.item._id)}
                            xs={{ height: "160%", transform: "rotate(90deg)", opacity: props.isLast ? "0.5" : "1", pointerEvents: props.isLast ? "none" : "all" }}
                        >
                            {ICONS.expand}
                        </IconAction>
                    </div>
            ...
            {!props.item._componentCollapsed &&
                props.item.subcomponents.map((item, index) => (
                    <Subcomponent 
                        item={item} 
                        selected={props.selected}
                        isFirst={index === 0}
                        isLast={index === props.item.subcomponents.length - 1}
                    />
                ))}
        </>
    );
        ...
        <>
            ...
            >
                ...
                <div className='-sidebar-component-icons-wrapper'>
                    ...
                    <div className="-ICON" style={{ height: "80%" }}>
                        <IconAction
                            x={() => subcomponents.moveItem("up", props.item._id)}
                            xs={{ height: "170%", transform: "rotate(-90deg)", opacity: props.isFirst ? "0.5" : "1", pointerEvents: props.isFirst ? "none" : "all"}}
                        >
                            {ICONS.expand}
                        </IconAction>
                        <IconAction
                            x={() => subcomponents.moveItem("down", props.item._id)}
                            xs={{ height: "170%", transform: "rotate(90deg)", opacity: props.isLast ? "0.5" : "1", pointerEvents: props.isLast ? "none" : "all" }}
                        >
                            {ICONS.expand}
                        </IconAction>
                    </div>
